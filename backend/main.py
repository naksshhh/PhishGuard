...
async def analyze_cloud(request: AnalysisRequest):
    ...
    # Tier 2 only runs if features are provided by the extension
    if request.features:
        ...
        t2_score = 0.5 # Default if no features

    is_blacklisted, sb_reason = sb_result

    # 2. Threshold check for Tier 1.5 (Safe Browsing Hit)
    if is_blacklisted:
        return AnalysisResponse(verdict="PHISH", score=1.0, tier=1, reason=f"CRITICAL: {sb_reason}")

    # 3. Handle Tier 2 (LightGBM) Results
    explanation = ""
    if request.features and (t2_score > 0.8 or t2_score >= 0.5):
        try:
    ...
    if t2_score > 0.8:
        return AnalysisResponse(verdict="PHISH", score=t2_score, tier=2, reason=explanation or "High structural risk.")
    
    # 4. Escalate to Tier 3 (Gemini) if ambiguous
    if t2_score >= 0.5:
        verdict, score, vision_reason = run_tier3_gemini(request.url, request.htmlExcerpt, request.screenshotBase64)
        final_reason = f"{vision_reason}\n\nTechnical Signals:\n{explanation}" if explanation else vision_reason
        return AnalysisResponse(verdict=verdict, score=score, tier=3, reason=final_reason)
